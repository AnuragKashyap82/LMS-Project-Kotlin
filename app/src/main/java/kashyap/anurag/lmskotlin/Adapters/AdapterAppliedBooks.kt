...
import kashyap.anurag.lmskotlin.databinding.RowBooksAppliedBinding
import java.util.*

class AdapterAppliedBooks : RecyclerView.Adapter<AdapterAppliedBooks.HolderAppliedBooks> {

    private val context: Context
    public var appliedBookArrayList: ArrayList<ModelAppliedBook>
    private var ADMIN_CODE: String? = ""

    private lateinit var binding: RowBooksAppliedBinding

    constructor(
        context: Context,
        appliedBookArrayList: ArrayList<ModelAppliedBook>,
        ADMIN_CODE: String?
    ) : super() {
        this.context = context
        this.appliedBookArrayList = appliedBookArrayList
        this.ADMIN_CODE = ADMIN_CODE
    }


    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): AdapterAppliedBooks.HolderAppliedBooks {
        binding = RowBooksAppliedBinding.inflate(LayoutInflater.from(context), parent, false)
        return HolderAppliedBooks(binding.root)
    }

    override fun onBindViewHolder(holder: AdapterAppliedBooks.HolderAppliedBooks, position: Int) {
        val model = appliedBookArrayList[position]
        val uid = model.uid
        val appliedDate = model.appliedDate
        val timestamp = model.timestamp

        binding.issuedDateTv.setText(appliedDate)
    ...
    private fun loadBookDetails(holder: HolderAppliedBooks, timestamp: String) {
    ...
    private fun showIssueBookDialog(timestamp: String, uid: String) {
    ...
    private fun issueBook(timestamp: String, uid: String, progressBar: ProgressBar, dialog: AlertDialog)
       {
    ...
    override fun getItemCount(): Int {
        return appliedBookArrayList.size
    }

    inner class  HolderAppliedBooks(itemView: View): RecyclerView.ViewHolder(itemView){
